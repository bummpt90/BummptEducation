...
| `src/data/attendanceData.ts` | `ALL_CLASSES_DEFINITIONS` (KG 1 to SSS 3 class tiers) | **B. REFERENCE/CONFIGURATION DATA** | Pure reference class structure. Live attendance records strictly retrieved via PostgreSQL. |
| `src/data/attendanceData.ts` | `TERM_CALENDAR_DAYS` (13-week term calendar) | **B. REFERENCE/CONFIGURATION DATA** | Official calendar reference; synthetic daily generator disabled (`getStoredAttendanceRecords` returns `{}`). |
